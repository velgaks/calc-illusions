...
//   2. joint share з ESS (зважено) — включаючи зріст (height в R11 пряма змінна)
//   3. shareFinal = joint
//   4. Wilson CI на effective sample size
...
export function computeAll(state, data) {
  ...
  // 4. Підсумок = joint (зріст уже у складі joint, не окремий множник)
  const shareFinal = jointShare;
  const countFinal = shareFinal * nBase;

  // 5. Wilson CI на effective sample size
  const bs = bootstrapShareCi(win, criteria);
  const ci = { low: bs.low, high: bs.high };
